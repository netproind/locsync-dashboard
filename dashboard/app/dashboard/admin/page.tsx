...
export default function AdminPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      } catch (e) { console.error('Twilio usage error:', e) }

      try {
        const res = await fetch(`/api/admin/make-usage`)
        if (res.ok) setMakeUsage(await res.json())
      } catch (e) { console.error('Make usage error:', e) }
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'20px',marginBottom:'24px'}}>
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'12px',padding:'24px'}}>
          ...
          {twilioUsage ? (
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              {[
                ['Voice Calls', String(twilioCalls), false],
                ['Voice Minutes', `${twilioMinutes} min`, false],
                ['SMS Sent', String(twilioSMS), false],
                ['Est. Cost', `$${Number(twilioCost).toFixed(2)}`, Number(twilioCost) > 50],
            ...
            <div style={{color:'var(--text-muted)',fontSize:'13px',textAlign:'center',padding:'20px 0'}}>Loading Twilio data...</div>
          )}
        </div>
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'12px',padding:'24px'}}>
          ...
          {makeUsage ? (
            ...
            <div style={{color:'var(--text-muted)',fontSize:'13px',textAlign:'center',padding:'20px 0'}}>Loading Make data...</div>
          )}
        </div>
      </div>
